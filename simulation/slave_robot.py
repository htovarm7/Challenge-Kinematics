...
def impedance_control(q, dq, x_des, dx_des, q_des_prev, F_contact=None, kd=KD_IMP, bd=BD_IMP):
    x_cur = fk_3r(q)
    J = jacobian_3r(q)
...
def setup_slave_plots(robot):
    ...
    ax_robot.plot(*HOLE_CENTER, 'x', color='#FFD700', markersize=10, markeredgewidth=2, zorder=3)
    ...
    ax_force.legend(loc='upper right', fontsize=8, facecolor='#1a1a2e', labelcolor='white')
    ...
    ax_tau.legend(loc='upper right', fontsize=8, facecolor='#1a1a2e', labelcolor='white')
    ...
    line_ex, = ax_err.plot([], [], color=C[0], linewidth=1.8, label='|e_x|')
